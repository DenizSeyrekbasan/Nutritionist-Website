"use client";
import { Disclosure } from "@headlessui/react";
import { ChevronUpIcon } from "@heroicons/react/20/solid";

const FAQ = () => {
  return (
    <div
      id="faq-section"
      className="mx-auto max-w-7xl py-24 lg:px-8 bg-faqblue rounded-2xl my-16 faq-bg"
    >
      <h3 className="text-xl font-normal text-white text-center mb-6">SSS</h3>
      <h2 className="text-4xl lg:text-6xl font-semibold text-center text-white">
        Sık Sorulan <br /> Sorular
      </h2>
      <div className="w-full px-4 pt-16">
        <div className="mx-auto w-full max-w-5xl rounded-2xl bg-white py-8 px-6 mb-5">
          <Disclosure as="div" className="mt-2">
            {({ open }) => (
              <>
                <Disclosure.Button className="flex w-full justify-between rounded-lg px-4 py-2 text-left text-2xl font-medium">
                  <span> Online diyet nedir ?</span>
                  <ChevronUpIcon
                    className={`${
                      open ? "rotate-180 transform" : ""
                    } h-5 w-5 text-purple-500`}
                  />
                </Disclosure.Button>
                <Disclosure.Panel className="px-4 pt-4 pb-2 text-base text-black font-normal opacity-50">
                  Online diyet mesafelerin önemsiz olduğu ve yanınızda her zaman
                  danışabileceğiniz diyetisyeninizin olduğu beslenme
                  programıdır. İstek ve ihtiyaçlarınız doğrultusunda hazırlanan
                  diyetiniz haftalık ölçüm ve görüşmeler ile takip altında
                  tutularak sürdürülebilir beslenmeyi hayatınıza adapte etmeyi
                  hedeflemektedir.
                </Disclosure.Panel>
              </>
            )}
          </Disclosure>
        </div>

        <div className="mx-auto w-full max-w-5xl rounded-2xl bg-white py-8 px-6 mb-5">
          <Disclosure as="div" className="mt-2">
            {({ open }) => (
              <>
                <Disclosure.Button className="flex w-full justify-between rounded-lg px-4 py-2 text-left text-2xl font-medium">
                  <span> Kan Tahlili Neden Gereklidir ?</span>
                  <ChevronUpIcon
                    className={`${
                      open ? "rotate-180 transform" : ""
                    } h-5 w-5 text-purple-500`}
                  />
                </Disclosure.Button>
                <Disclosure.Panel className="px-4 pt-4 pb-2 text-base text-black font-normal opacity-50">
                  Kan tahlili vücudunuzun gereksinim ve fazlalıklarını açık bir
                  şekilde ifade eden biyokimyasal bulguların dökümüdür. Kişiye
                  özel beslenme programı oluştururken bu markerların göz önünde
                  bulundurulması önemli bir husustur.
                </Disclosure.Panel>
              </>
            )}
          </Disclosure>
        </div>

        <div className="mx-auto w-full max-w-5xl rounded-2xl bg-white py-8 px-6 mb-5">
          <Disclosure as="div" className="mt-2">
            {({ open }) => (
              <>
                <Disclosure.Button className="flex w-full justify-between rounded-lg px-4 py-2 text-left text-2xl font-medium">
                  <span> Anamnez formu nedir ? Neden Gereklidir ?</span>
                  <ChevronUpIcon
                    className={`${
                      open ? "rotate-180 transform" : ""
                    } h-5 w-5 text-purple-500`}
                  />
                </Disclosure.Button>
                <Disclosure.Panel className="px-4 pt-4 pb-2 text-base text-black font-normal opacity-50">
                  Anamnez formu; kişinin beslenme alışkanlıkları,sağlık
                  geçmişi,yaşam standartları hakkında bilgi vermektedir. Bu form
                  sayesinde bireyin uygun tedavi ve beslenme programı doğru bir
                  şekilde hazırlanmaktadır.
                </Disclosure.Panel>
              </>
            )}
          </Disclosure>
        </div>

        <div className="mx-auto w-full max-w-5xl rounded-2xl bg-white py-8 px-6 mb-5">
          <Disclosure>
            {({ open }) => (
              <>
                <Disclosure.Button className="flex w-full justify-between rounded-lg px-4 py-2 text-left text-2xl font-medium">
                  <span>
                    Hangi besin gruplarını içeren bir diyet programı
                    öneriyorsunuz?
                  </span>
                  <ChevronUpIcon
                    className={`${
                      open ? "rotate-180 transform" : ""
                    } h-5 w-5 text-purple-500`}
                  />
                </Disclosure.Button>
                <Disclosure.Panel className="px-4 pt-4 pb-2 text-base text-black font-normal opacity-50">
                  Sağlıklı bir diyet programı genellikle tüm besin gruplarını
                  içermelidir. Dengeli bir yaklaşım, proteinler,
                  karbonhidratlar, yağlar, vitaminler ve mineraller açısından
                  zengin bir beslenmeyi hedefler.
                </Disclosure.Panel>
              </>
            )}
          </Disclosure>
        </div>

        <div className="mx-auto w-full max-w-5xl rounded-2xl bg-white py-8 px-6 mb-5">
          <Disclosure as="div" className="mt-2">
            {({ open }) => (
              <>
                <Disclosure.Button className="flex w-full justify-between rounded-lg px-4 py-2 text-left text-2xl font-medium">
                  <span>
                    Hızlı kilo vermek için en etkili stratejiler nelerdir?
                  </span>
                  <ChevronUpIcon
                    className={`${
                      open ? "rotate-180 transform" : ""
                    } h-5 w-5 text-purple-500`}
                  />
                </Disclosure.Button>
                <Disclosure.Panel className="px-4 pt-4 pb-2 text-base text-black font-normal opacity-50">
                  Hızlı kilo vermek yerine sağlıklı ve sürdürülebilir bir kilo
                  verme hedeflemek önemlidir. Ani kilo kaybı genellikle vücut
                  için zararlı olabilir. Sağlıklı kilo vermek için porsiyon
                  kontrolü, düzenli egzersiz, su tüketimi artırma, işlenmiş
                  gıdalardan kaçınma ve uzun vadeli beslenme alışkanlıklarını
                  geliştirmek önerilir. Kısa vadeli çözümler yerine yaşam tarzı
                  değişiklikleri daha etkili sonuçlar doğurabilir.
                </Disclosure.Panel>
              </>
            )}
          </Disclosure>
        </div>

        <div className="mx-auto w-full max-w-5xl rounded-2xl bg-white py-8 px-6 mb-5">
          <Disclosure as="div" className="mt-2">
            {({ open }) => (
              <>
                <Disclosure.Button className="flex w-full justify-between rounded-lg px-4 py-2 text-left text-2xl font-medium">
                  <span> Detoks suyuyla zayıflayabilir miyim ?</span>
                  <ChevronUpIcon
                    className={`${
                      open ? "rotate-180 transform" : ""
                    } h-5 w-5 text-purple-500`}
                  />
                </Disclosure.Button>
                <Disclosure.Panel className="px-4 pt-4 pb-2 text-base text-black font-normal opacity-50">
                  Hayır. Mucizevi bir su ve besin yoktur. Tek çeşit beslenmek
                  insan fizyolojisine aykırıdır. Makro ve mikro besin
                  ihtiyaçlarını çeşitlilik sağlayarak tüketmek hem daha
                  sürdürülebilir hem de daha sağlıklı bir modeldir.
                </Disclosure.Panel>
              </>
            )}
          </Disclosure>
        </div>
      </div>
    </div>
  );
};

export default FAQ;
