        ...
        <div className="mx-auto w-full max-w-5xl rounded-2xl bg-white py-8 px-6">
          <Disclosure as="div" className="mt-2">
            {({ open }) => (
              <>
                <Disclosure.Button className="flex w-full justify-between rounded-lg px-4 py-2 text-left text-2xl font-medium">
                  <span> Spor yapmadan da kilo verebilir miyim?</span>
                  <ChevronUpIcon
                    className={`${
                      open ? "rotate-180 transform" : ""
                    } h-5 w-5 text-purple-500`}
                ...
                <Disclosure.Panel className="px-4 pt-4 pb-2 text-base text-black font-normal opacity-50">
                  Evet, spor yapmadan da kilo verebilirsiniz, ancak sporun kilo
                  verme sürecine olumlu etkileri vardır. Kiloyu dengelemek için
                  kalori alımını kontrol etmek önemlidir. Düzenli egzersiz
                  metabolizmayı artırabilir, kas kütlesi oluşturabilir ve kilo
                  verme sürecini hızlandırabilir. Ancak, sadece beslenmeye
                  odaklanarak da kilo verebilirsiniz. Sağlıklı besin seçimleri
                  ve porsiyon kontrolü kilo yönetiminde büyük rol oynar.
                </Disclosure.Panel>
              </>
            )}
          </Disclosure>
